...
//TODO - Integrar com o schema (gerente de db)
		...
		abstract class Model extends Properties{
			...
			function resetCommand(){
				$this->sql = array( 'field'  => array(), 'where'  => array(), 'order'  => array(), 'group'  => array(), 'join'   => array(), 'limit' => array());
			}
			...
			function countData( $key ){
				...
				$sql   = 'SELECT 1=1 FROM `' . $this->table . '`';
			...
			function saveRow( $row, $table = '', $retCommand = false ){
				...
				foreach( $structure as $col ){
					...
					if( !(($col->is_auto_increment && empty($row[ $col->name ]->value)) || $col->field_type == 'timestamp') ){
						
						//Verifica se é chave primaria e tem um valor
						if( $col->is_primary_key && empty( $row[ $col->name ]->value ) ){
						...
						if( $col->accept_null == false &&
							 $col->is_auto_increment == false &&
							 	(is_null( $row[ $col->name ]->value ) || $row[ $col->name ]->value === 'null') ){
				...
				if( $conn->execute( $sql ) ){
   				...
   				$sql = $where = '';
					...
					foreach( $pk as $i => $a ){
						...
						if( $structure[ $i ]->is_auto_increment && (empty( $a ) || $a == 'null') ){
							$a = $conn->lastId;
						}
					...
					throw( new ModelException( "Error while saving", 2105 ) );
            ...
            if( strpos( '[blog][mediumblob][bigblob][tinyblob][smallblob][text][mediumtext][bigtext][tinytext][smalltext]', $field->format->field_type ) ){
               ...
               $field->value = $tools->antiInjection( $field->value );  
				...
				if( $field->format->field_type == 'datetime' &&
			...
			function sqlField( $field ){
				...
				if( strpos( '[datetime][integer][decimal][double][float][int][smallint][bigint][tinyint][unsigned]', $field->format->field_type ) === false ){
					$ret = '"' . $field->value . '"';
